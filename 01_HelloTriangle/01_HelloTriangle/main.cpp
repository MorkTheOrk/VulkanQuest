...
private:
	...
	void createGraphicsPipeline() {
		...
		// No VERTEX definitions and attributes -> change this for vertex buffer
		...
		rasterizer.depthClampEnable = VK_FALSE; // discard pixel that dont pass depth test
		rasterizer.rasterizerDiscardEnable = VK_FALSE; // block any geometry to be send to the rasterizer
		rasterizer.polygonMode = VK_POLYGON_MODE_FILL; // how to draw polygons -> fill, line, point
		rasterizer.lineWidth = 1.0f;
		//culling
		rasterizer.cullMode = VK_CULL_MODE_BACK_BIT;
		rasterizer.frontFace = VK_FRONT_FACE_CLOCKWISE;

		// add constant value to depth buffer
		rasterizer.depthBiasEnable = VK_FALSE;
		rasterizer.depthBiasConstantFactor = 0.0f; // Optional
		rasterizer.depthBiasClamp = 0.0f; // Optional
		rasterizer.depthBiasSlopeFactor = 0.0f; // Optional

		// multisample is disables -> needs a GPU feature to be enabled
		...
		// no depth or stencil buffer yet
		// ...

		// Colorblending -> how to combine colors in framebuffers. currently = nA * nC + (1- nA) * oC
		...
		// attach colorblending into the pipeline
	...
	void createRenderPass() {
		...
		colorAttachment.samples = VK_SAMPLE_COUNT_1_BIT;		// sample 1 time cuz no multisample enabled

		// how to attach data to a framebuffer
		colorAttachment.loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR;	// clear values to a constant before rendering
		colorAttachment.storeOp = VK_ATTACHMENT_STORE_OP_STORE; // result will be stored in memory
	...
	void drawFrame() {
		...
		VkSemaphore waitSemaphores[] = { imageAvailableSemaphores[currentFrame] }; // wait for image to be available
		...
		VkSemaphore signalSemaphores[] = { renderFinishedSemaphores[currentFrame] }; // wait for image to be rendered
	...
	void cleanup() {
		...
		vkDestroyCommandPool(device, commandPool, nullptr); // REMOVE COMMAND POOL
	...
	std::vector<VkSemaphore> imageAvailableSemaphores; // sync to get new image from swap chain for each image
	std::vector<VkSemaphore> renderFinishedSemaphores; // sync to push render image into swap chain
	std::vector<VkFence> inFlightFences; //sync cpu with gpu
